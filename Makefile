...
.PHONY: clobber
clobber: clean
	find . -type d -name '$(doc_build_dir)' | xargs -n 1 rm -rf
	find . -type d -name 'htmlcov' | xargs -n 1 rm -rf
	find . -type d -name '.pytest_cache' | xargs -n 1 rm -rf
	find . -type d -name '.tox' | xargs -n 1 rm -rf
...
.PHONY: clean
clean:
	find . -type d -name 'build' | xargs -n 1 rm -rf
	find . -type d -name '__pycache__' | xargs -n 1 rm -rf
	find . -type d -name '.cache' | xargs -n 1 rm -rf
	find . -type d -name '$(package_name_under).egg-info' | xargs -n 1 rm -rf
	find . -type d -name '.eggs' | xargs -n 1 rm -rf
	rm -f MANIFEST MANIFEST.in AUTHORS ChangeLog .coverage
	find . -type f -name '*.pyc *.tmp tmp_*' -delete
